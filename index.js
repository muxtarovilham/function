...
function findCountryById(orders, targetId) {
    for (let i = 0; i < orders.length; i++) {
      if (orders[i].id === targetId) {
        return orders[i].shipAddress.country;
      }
    }
  }

  
const targetId = 10279;
const country = findCountryById(orders, targetId);
...
function findOrdersByCountry(orders, targetCountry) {
    const foundOrders = [];
    
    for (let i = 0; i < orders.length; i++) {
      if (orders[i].shipAddress.country === targetCountry) {
...
const targetCountry = "Austria";
const foundOrders = findOrdersByCountry(orders, targetCountry);

if (foundOrders.length > 0) {
  console.log(`"${targetCountry}" ölkəsinə görə əmrlər:`);
  foundOrders.forEach(order => {
    console.log(`ID: ${order.id}, ShipName: ${order.shipName}`);
  });
} else {
  console.log(`Belə bir ölkəyə görə sifariş tapılmadı.`);
}
...
function findShipDetailsById(orders, id) {
    const order = orders.find(order => order.id === id);
  
    if (order) {
      const shipName = order.shipName;
      const country = order.shipAddress.country;
      const city = order.shipAddress.city;
  
      return { shipName, country, city };
    } else {
      return null; 
    }
  }
...
const targetid = 10272; 
const shipDetails = findShipDetailsById(orders, targetid);

if (shipDetails) {
  console.log(`ID ${targetid} üçün məlumatlar:`);
  console.log(`ShipName: ${shipDetails.shipName}`);
  console.log(`Country: ${shipDetails.country}`);
  console.log(`City: ${shipDetails.city}`);
} else {
...
function countProductTypesById(orders, id) {
    const order = orders.find(order => order.id === id);
  
    if (order) {
      // Əgər sifariş tapılıbsa, details array-inin uzunluğunu hesablamaq üçün length istifadə edirik.
      const numberOfProductsSold = order.details.length;
      return numberOfProductsSold;
    } else {
      return 0; // Əgər verilmiş ID-ə uyğun sifariş tapılmazsa, 0 qaytarırıq.
    }
  }


  const targettId = 10271; // Axtarış üçün istənilən ID-ni təyin edin
const productCount = countProductTypesById(orders, targettId);

if (productCount > 0) {
  console.log(`ID ${targettId} üçün satılan məhsulların növü: ${productCount}`);
} else {
  console.log(`Belə bir ID-ə görə sifariş tapılmadı və ya heç bir məhsul satılmayıb.`);
}
  ...
  function myFunction (id) {
    ...
    if (order) {
      for (const detail of orders) {
        totalProfit += (order.details.unitPrice * order.details.quantity) * (1 - order.details.discount);
      }
    }
